...
```
이름·비밀번호  →  메일로 온 6자리 인증번호  →  초대코드  →  바로 이용
                                                (24시간, 최대 10명)
                                              ↓ 코드가 없거나 만료됐으면
                                                승인 대기 (관리자가 승인)
```

- **초대코드는 메일 인증을 마친 뒤에 넣습니다.** 코드로 계정 만들기를 막아두면
  코드를 받으러 간 사이에 적어둔 것이 다 날아가고 처음부터 다시 적어야 했습니다
- **메일 인증은 링크가 아니라 6자리 인증번호**입니다. 가입 화면에서 그대로 넣으면 끝납니다
- 코드 없이 만들어진 계정은 **승인 대기**로 남아 아무것도 하지 못합니다.
  들어오는 문은 화면이 아니라 **DB의 RLS 정책**이 지키므로, 가입 요청을 직접 보내도 달라지지 않습니다
- 코드의 사용 횟수는 **인증을 마친 뒤에** 올라갑니다 —
  인증하지 않은 사람이 남의 몫을 축내지 않게 하기 위해서입니다
- **거절과 차단은 초대코드로 풀리지 않습니다.** 되돌리는 것은 관리자만 합니다
