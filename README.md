...
4. `폰으로 보내기 (QR 만들기)` — 밴드에 올리고, QR은 받아서 새가족부에 전달
...
내보낸 뒤 `폰으로 보내기 (QR 만들기)`를 누르면 이미지를 서버에 올리고 QR을 만듭니다.
QR을 찍으면 `/view/<주보번호>`가 열려 **주보가 첫 장부터 순서대로** 나옵니다.

- QR 주소는 주보 번호로 고정됩니다. 내용을 고쳐 다시 올려도 **이미 뿌린 QR은 그대로** 쓸 수 있습니다.
- 화면 QR과 별개로 PNG를 512 / 1024 / 2048 / **2480(A4 300dpi)** px 중에 골라 받습니다.
  고른 크기로 다시 그리므로 키워도 흐려지지 않습니다.
- 올린 이미지는 서버의 `.shares/` 폴더에 있습니다 (git에 올라가지 않음).

> 지금은 이 컴퓨터가 서버라, **같은 와이파이에 있는 폰에서만** 열립니다.
> QR에는 `localhost`가 아니라 이 컴퓨터의 랜 주소가 자동으로 들어갑니다.
> 교회 밖에서도 열리게 하려면 서버를 어딘가에 올려야 합니다 (3단계).
...
src/
  app/
    ...
    library/           보관함
    view/[id]/         QR을 찍은 폰이 보는 화면
    api/share/[id]/    폰에 보여줄 이미지 올리기·내려받기
  components/
  ...
  lib/
    ...
    shareStore.ts      폰에 보여줄 이미지 보관 (서버 .shares 폴더)
...
| 3단계 | 테마(배경 세트) 관리 · 서버 배포(QR을 교회 밖에서도 열리게) |
