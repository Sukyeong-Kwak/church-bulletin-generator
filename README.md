...
2. `SQL Editor`에서 `supabase/migrations/001_init.sql` → `002_share.sql` → `003_email_and_block.sql` 순서로 실행
3. `Authentication`에서 **Confirm email** 켜두기 (가입하려면 메일 링크를 눌러야 합니다)
...
이름·이메일·비밀번호로 신청하면 관문을 둘 지나야 합니다.

```
가입 신청 → 메일 링크 확인 ──┬── 초대코드 입력  →  바로 이용 (24시간 유효, 최대 10명)
 (이름·이메일·비밀번호)      └── 코드 없음      →  승인 대기  →  관리자가 승인  →  이용
```

- **메일 인증을 마쳐야 로그인됩니다.** 남의 메일 주소로 신청할 수 없습니다.
  메일이 안 왔으면 로그인 화면에서 다시 보낼 수 있습니다
- 가입할 때 적은 초대코드는 메일 링크를 누른 뒤 **자동으로 쓰입니다** —
  그 시점에는 아직 로그인 전이라 코드를 쓸 수 없기 때문입니다
- 접근 통제는 화면이 아니라 **DB의 RLS 정책**으로 막습니다. 주소를 직접 쳐도 승인 전에는 아무것도 보이지 않습니다
- 관리자는 상단 **관리자** 메뉴에서 승인·거절, 초대코드 발급·폐기, 역할 변경, **로그인 차단**을 합니다

### 관리자가 하는 일

| | 어디서 보나 | 하는 일 |
|---|---|---|
| **메일 인증 여부** | 이름 옆의 초록/주황 배지 | 인증 전인 사람은 승인 버튼이 잠깁니다 |
| **승인 · 거절** | 가입 신청 목록 | 승인해야 주보를 만들 수 있습니다 |
| **로그인 차단** | 구성원 목록 | 쓰던 사람을 즉시 막습니다. 계정과 주보는 남고, 차단 해제하면 그대로 이어 씁니다 |

> 관리자는 **자기 자신을 차단하거나 역할을 낮출 수 없습니다.** 화면에서 버튼을 감출 뿐 아니라
> DB 정책으로도 막아두었습니다 — 마지막 관리자가 스스로를 잠그면 되살릴 사람이 없기 때문입니다.
