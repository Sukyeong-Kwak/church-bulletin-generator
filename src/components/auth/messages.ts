...
export function redeemFailMessage(status: string | null | undefined): string {
  // 거절·차단은 코드로 풀리지 않는다. 되돌리는 것은 관리자의 몫이다.
  // '코드가 틀렸다'고만 하면 멀쩡한 코드를 몇 번이고 다시 넣어보게 된다.
  if (status === "rejected")
    return "이 계정은 가입이 거절되었습니다. 초대코드로는 열리지 않습니다. 담당자에게 문의해주세요.";

  if (status === "blocked")
    return "관리자가 이 계정의 이용을 중지했습니다. 초대코드로는 열리지 않습니다. 담당자에게 문의해주세요.";
