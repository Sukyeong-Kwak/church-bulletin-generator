...
export interface FixedPages {
  ...
  worship: {
    ...
    /**
     * 생일 명단을 한 줄에 몇 명씩 세울지 (1~5).
     * 비어 있으면 남은 자리를 재서 스스로 고른다 — 대개는 그것으로 충분하다.
     * 달마다 사람 수가 크게 달라 눈에 거슬릴 때만 손으로 잡는다.
     */
    birthdayCols?: number;
    /**
     * 생일 이름 글자 크기(px).
     * 비어 있으면 고른 짜임이 남은 자리에 들어가도록 스스로 줄인다.
     * 값을 넣으면 그 크기를 그대로 쓴다 — 카드를 넘치더라도 줄이지 않는다.
     */
    birthdayNameSize?: number;
    /**
